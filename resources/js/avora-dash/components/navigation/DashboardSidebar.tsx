...
import { dashboardAccountNavigation, dashboardNavigation } from '../../data/navigation';
...
export function DashboardSidebar({ open, desktopOpen, onClose }: DashboardSidebarProps) {
    ...
    return (
        <>
            <button
                type="button"
                aria-label={translate({ ar: 'إغلاق القائمة', en: 'Close menu' })}
                onClick={onClose}
            ...
            >
                <div className="flex h-20 items-center justify-between border-b border-slate-100 px-6 dark:border-slate-800">
                    <Link href="/dashboard" className="flex items-center gap-3" onClick={onClose}>
                        {logoUrl ? (
                            <img src={logoUrl} alt={appName} className="h-10 w-10 rounded-xl object-contain" />
                        ) : (
                            <span className="avora-button-primary grid h-10 w-10 place-items-center rounded-xl text-lg font-black text-white">{appName.charAt(0).toUpperCase()}</span>
                        )}
                        <span>
                            <span className="block text-lg font-bold tracking-tight text-slate-950 dark:text-white">{appName}</span>
                ...
                <nav className="flex-1 overflow-y-auto px-4 py-6">
                    <p className="mb-3 px-3 text-xs font-semibold uppercase tracking-wider text-slate-400">{translate({ ar: 'القائمة الرئيسية', en: 'Main menu' })}</p>
                    <ul className="space-y-1">{renderItems(dashboardNavigation)}</ul>
                    <p className="mb-3 mt-8 px-3 text-xs font-semibold uppercase tracking-wider text-slate-400">{translate({ ar: 'الحساب', en: 'Account' })}</p>
                    <ul className="space-y-1">{renderItems(dashboardAccountNavigation)}</ul>
                </nav>
