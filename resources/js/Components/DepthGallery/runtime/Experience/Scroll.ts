...
class Scroll {
  constructor(camera, gallery, debug = null, host = null) {
    ...
    this.debug = debug
    this.host = host || null
    this.eventTarget = this.host || window
  ...
  bindEvents() {
    ...
    this.eventTarget.addEventListener('touchmove', this.onTouchMove, { passive: false })
  }
  ...
  createVelocityVisualizer() {
    ...
    ;(this.host || document.body).append(container)
