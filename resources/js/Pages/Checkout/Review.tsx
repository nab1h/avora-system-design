...
export default function Review() {
    ...
    return (
        <>
            <Head
                title={translate({ ar: "مراجعة الطلب", en: "Review order" })}
            />

            <main
                className="min-h-screen bg-slate-50 py-10 dark:bg-slate-950"
                dir={direction}
            >
                <div className="mx-auto grid max-w-5xl gap-6 px-5 lg:grid-cols-[1fr_360px]">
                    ...
                    <aside className="h-fit rounded-2xl bg-white p-6 shadow-sm dark:bg-slate-900">
                        ...
                        <div className="mt-6 flex justify-between border-t pt-4 text-lg font-black">
                            ...
                            <span>
                                {Number(props.total).toFixed(2)} {currency}
                            </span>
                        </div>
                        <Button
                            fullWidth
                            className="mt-5"
                            onClick={() => router.post(route("checkout.store"))}
                        >
                            {translate({
                                ar: "تأكيد والدفع",
                                en: "Confirm & pay",
            ...
            <Modal
                ...
                title={translate({
                    ar: "تعديل بيانات الشحن",
                    en: "Edit shipping details",
                })}
                size="lg"
            >
                <form onSubmit={saveAddress} className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <FormField
                            required
                            label={translate({ ar: "الهاتف", en: "Phone" })}
                        ...
                        <FormField
                            required
                            label={translate({ ar: "المدينة", en: "City" })}
                        ...
                        />
                        <FormField
                            label={translate({ ar: "المنطقة", en: "Area" })}
                        ...
                        <FormField
                            required
                            label={translate({ ar: "الشارع", en: "Street" })}
                        ...
                        />
                        <FormField
                            label={translate({ ar: "المبنى", en: "Building" })}
                            value={form.data.building}
                            onChange={(event) =>
                                form.setData("building", event.target.value)
                            }
                        />
                    </div>
