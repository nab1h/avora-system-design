...
export default function CheckoutSuccess({ transaction }: CheckoutResultProps) {
    ...
    return (
        <>
            <Head title={translate({ ar: "تم الدفع بنجاح", en: "Payment successful" })} />
                ...
                <Container className="space-y-6">
                    ...
                    <Card variant="elevated" padding="lg" className="mx-auto max-w-2xl text-center">
                        <CardHeader>
                            ...
                            <CardMeta className="mt-5">{transaction.uuid}</CardMeta>
                            <CardTitle>
                                {translate({ ar: "العملية تمت بنجاح", en: "Payment completed successfully" })}
                            </CardTitle>
                        </CardHeader>
                        ...
                        <CardContent className="space-y-5">
                            ...
                            <div className="rounded-2xl bg-slate-100 p-5 text-start text-sm dark:bg-slate-900">
                                <div className="flex justify-between gap-4">
                                    <span className="text-slate-500">{translate({ ar: "المنتج", en: "Product" })}</span>
                                    <span className="font-bold">{transaction.product_name}</span>
                                </div>
                                <div className="mt-3 flex justify-between gap-4">
                                    <span className="text-slate-500">{translate({ ar: "المبلغ", en: "Amount" })}</span>
                                    <span className="font-bold">{transaction.amount_decimal} {transaction.currency}</span>
                                </div>
                                <div className="mt-3 flex justify-between gap-4">
                                    <span className="text-slate-500">{translate({ ar: "البوابة", en: "Gateway" })}</span>
                                    <span className="font-bold">{transaction.gateway_name ?? transaction.gateway_slug}</span>
                                </div>
                            </div>

                            <div className="flex flex-wrap justify-center gap-3">
                                <Link href={route("home")}>
                                    <Button>{translate({ ar: "الرجوع للموقع", en: "Back to website" })}</Button>
                                </Link>
                                <Link href={route("checkout.show", transaction.uuid)}>
                                    <Button variant="outline">{translate({ ar: "تفاصيل الطلب", en: "Order details" })}</Button>
                                </Link>
                            </div>
                        </CardContent>
                    </Card>
