...
export default function Shopping() {
    ...
    const [category, setCategory] = useState<number | null>(
        Number(initialParams.get("category")) || null,
    );
    const [subcategory, setSubcategory] = useState<number | null>(
        Number(initialParams.get("subcategory")) || null,
    );
    ...
    const visible = useMemo(
            ...
            products
                .filter(
                    (product) =>
                        ...
                        (!color ||
                            product.colors.some((item) => item.id === color)) &&
                        (!size ||
                            product.sizes.some((item) => item.id === size)),
                )
                .sort((a, b) =>
                    sort === "low"
        ...
        [products, category, subcategory, color, size, sort],
    );
    const FilterGroup = ({
        title,
        children,
    ...
    const FilterButton = ({
        ...
        <button
            onClick={onClick}
            className="mb-2 flex w-full items-center gap-2 text-start text-sm"
            ...
            >
                <Container width="wide" gutter="lg">
                    ...
                    <div className="flex flex-col gap-6 md:flex-row-reverse">
                        <aside
                            className="sticky top-6 h-fit shrink-0 self-start rounded-2xl border p-5 md:w-56"
                            style={{
                                ...
                                {visible.map((product) => {
                                    ...
                                    return (
                                        <ProductCard
                                            ...
                                            price={`${product.price} ${props.websiteSettings?.currency ?? "EGP"}`}
                                            img={
                                                image
                                                    ? `/storage/${image.image}`
                                            ...
                                            favoriteCount={
                                                product.favorited_by_users_count
                                            }
                                            onView={() =>
