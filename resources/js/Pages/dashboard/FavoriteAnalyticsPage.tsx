...
export function FavoriteAnalyticsPage() {
    ...
    const { favoriteAnalytics } =
        usePage<PageProps<{ favoriteAnalytics: Analytics }>>().props;
    ...
    const stats = [
        [
            translate({ ar: "إجمالي الإضافات للمفضلة", en: "Total favorites" }),
            favoriteAnalytics.total_items,
        ],
        [
            translate({
                ar: "عملاء لديهم مفضلة",
                en: "Customers with favorites",
            }),
            favoriteAnalytics.customers_count,
        ],
        [
            translate({ ar: "منتجات مفضلة", en: "Favorited products" }),
            favoriteAnalytics.products_count,
        ],
    ];
        ...
        <DashboardLayout>
            <header className="mb-6">
                ...
                <p className="mt-2 text-sm text-slate-500">
                    {translate({
                        ar: "تابع المنتجات التي يهتم بها عملاؤك أكثر.",
                        en: "Track the products your customers care about most.",
                    })}
                </p>
            ...
            >
                <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                    <div className="border-b border-slate-200 p-5 dark:border-slate-800">
                        <h2 className="font-bold text-slate-950 dark:text-white">
                            {translate({
                                ar: "الأكثر إضافة للمفضلة",
                                en: "Most favorited",
                            })}
                        </h2>
                    </div>
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                        {favoriteAnalytics.top_products.length ? (
                            favoriteAnalytics.top_products.map(
                                (product, index) => (
                                    <div
                                        key={product.id}
                                        className="flex items-center gap-3 p-4"
                                    >
                                        <span className="grid h-8 w-8 place-items-center rounded-full bg-rose-100 text-sm font-black text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">
                                            {index + 1}
                                        </span>
                                        {product.image ? (
                                            <img
                            ...
                            <p className="p-8 text-center text-sm text-slate-500">
                                {translate({
                                    ar: "لا توجد منتجات مفضلة بعد.",
                                    en: "No favorite activity yet.",
                                })}
                            </p>
                        )}
                    </div>
                ...
                <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                    <div className="border-b border-slate-200 p-5 dark:border-slate-800">
                        <h2 className="font-bold text-slate-950 dark:text-white">
                            {translate({
                                ar: "أحدث نشاط في المفضلة",
                                en: "Latest favorite activity",
                            })}
                        </h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[650px] text-sm">
                            ...
                            <tbody>
                                {favoriteAnalytics.items.length ? (
                                    favoriteAnalytics.items.map((item) => (
                                        <tr
                                            key={item.id}
